...
def Rig(module):
    ...
    if upperTwist=='True' or lowerTwist=='True':
        ...
        cmds.addAttr(extraCNT,longName='bendy',attributeType='float',
                     min=0,keyable=True,defaultValue=0)
    
    if upperTwist=='True':
        tj.Rig(startJoint, midJoint, prefix+'upper_', int(upperTwistJoints),
                      asset, plug, averageJNT,'end',extraCNT+'.bendy')
    ...
    if lowerTwist=='True':
        tj.Rig(midJoint,endJoint,prefix+'lower_', int(upperTwistJoints),
                      asset, plug, averageJNT,'start',extraCNT+'.bendy')
...
class TwistJoints():
    ...
    def Rig(self,startNode,endNode,prefix,jointAmount,asset,plug,bendNode,
            bendDirection,bendControl):
        ...
        #create pathSpine
        pathSpine=mpps.MG_pathSpine(curve, jointAmount-1,skinCurve=True)
        ...
        cmds.parentConstraint(endNode,iktwistend,mo=True)
        cmds.parentConstraint(startNode,iktwiststart,mo=True)
        ...
        dist=mru.Distance(startNode, endNode)
        
        if bendDirection=='end':
            
            cmds.setAttr(ikjoints[0]+'.sx',0.0001)
            cmds.setAttr(ikjoints[0]+'.sy',0.0001)
            cmds.setAttr(ikjoints[0]+'.sz',0.0001)
            
            cmds.parent(ikjoints[3],bendNode)
            mru.Snap(bendNode, ikjoints[3])
            
            cmds.setAttr(ikjoints[2]+'.tx',-dist/2)
            cmds.setAttr(ikjoints[2]+'.ty',0)
            cmds.setAttr(ikjoints[2]+'.tz',0)
            cmds.setAttr(ikjoints[2]+'.rx',0)
            cmds.setAttr(ikjoints[2]+'.ry',0)
            cmds.setAttr(ikjoints[2]+'.rz',0)
            
            cmds.connectAttr(bendControl,ikjoints[3]+'.sx')
            cmds.connectAttr(bendControl,ikjoints[3]+'.sy')
            cmds.connectAttr(bendControl,ikjoints[3]+'.sz')
        
        if bendDirection=='start':
            
            cmds.parent(ikjoints[3],endNode)
            
            cmds.setAttr(ikjoints[3]+'.sx',0.0001)
            cmds.setAttr(ikjoints[3]+'.sy',0.0001)
            cmds.setAttr(ikjoints[3]+'.sz',0.0001)
            
            cmds.parent(ikjoints[0],bendNode)
            mru.Snap(bendNode, ikjoints[0])
            
            cmds.setAttr(ikjoints[1]+'.tx',dist/2)
            cmds.setAttr(ikjoints[1]+'.ty',0)
            cmds.setAttr(ikjoints[1]+'.tz',0)
            cmds.setAttr(ikjoints[1]+'.rx',0)
            cmds.setAttr(ikjoints[1]+'.ry',0)
            cmds.setAttr(ikjoints[1]+'.rz',0)
            
            cmds.connectAttr(bendControl,ikjoints[0]+'.sx')
            cmds.connectAttr(bendControl,ikjoints[0]+'.sy')
            cmds.connectAttr(bendControl,ikjoints[0]+'.sz')
        
        #return
        return result

#something is wrong with the twist setup, needs to be more generic and any
#custom needs to happen outside of the method
