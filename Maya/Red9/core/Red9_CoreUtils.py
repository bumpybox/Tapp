...
def prioritizeNodeList(nList,priorityList,regex=True):
    '''
...
#    # property.setter is only valid from python2.6, therefore fails under Maya2009 
#    @property
#    def rootNodes(self):
#        if self._rootNodes:
#            return self._rootNodes
#        else:
#            raise StandardError('No RootNodes Set')    
#    @rootNodes.setter
#    def rootNodes(self, value):
#        if value:
#            self._rootNodes = value
#        else:
#            raise StandardError('no Value Given')    
    ...
    def lsSearchNodeTypes(self, nodeTypes, nodes=None, incRoots=True, transformClamp=False):
        ...
        if not isinstance(nodeTypes, list): nodeTypes = [nodeTypes]
    ...
    def lsSearchAttributes(self, searchAttrs, nodes=None, incRoots=True, returnValues=False):
        ...
        if not isinstance(searchAttrs,list): searchAttrs=[searchAttrs]
    ...
    def lsSearchNamePattern(self, searchPattern, nodes=None, incRoots=True): 
        ...
        include=[]
        exclude=[]
        if not isinstance(searchPattern, list): searchPattern=[searchPattern]
...
class LockChannels(object):
    ...
    def loadChannelMap(self,filepath=None,nodes=None,hierarchy=True,serializeNode=None):
        ...
        for node in nodes:
            key=nodeNameStrip(node)
            if self.statusDict.has_key(key):
...
class TimeOffset(object):
    ...
    @classmethod       
    def fromSelected(cls,offset,filterSettings=None,flocking=False,randomize=False):
        '''
        ...
        if selectedNodes:
            if flocking or randomize:
                ...
                cls.animCurves(offset, selectedNodes)
    ...
    @staticmethod 
    @r9General.Timer
    def animCurves(offset,nodes=None):
        '''
        ...
        if safeCurves:  
            ...
            try:
                ...
                #log.debug('Failed Offset : %s' % animCurve)
                pass
            log.debug('%i : AnimCurves were offset\n' % len(safeCurves))
    ...
    def sound(offset,mode='Scene',audioNodes=None):
        ...
        '''
        if mode=='Scene': audioNodes=cmds.ls(type='audio') 
        if audioNodes:
    ...
    def animClips(offset,mode='Scene',clips=None):
        ...
        '''
        if mode=='Scene': clips=cmds.ls(type='animClip') 
        if clips:
