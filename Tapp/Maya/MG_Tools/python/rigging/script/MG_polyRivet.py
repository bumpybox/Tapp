...
def MG_polyRivet (baseName = "polyRivet", mesh = None , target = None, mo = 1):
    '''
    This procedure is used to hook up an MG_polyRivet
    @param baseName : this is the name used for the node
    @param mesh : string ,the mesh we want to rivet on 
    ...
    if val == 1 :
        cmds.loadPlugin("MG_rigToolsPro",quiet=True)
    elif val2 == 1 :
        cmds.loadPlugin("MG_toolsLite",quiet=True) 
        
    shape = cmds.listRelatives(mesh , shapes =  1)[0]
    if cmds.nodeType(shape) != "mesh" :
        OpenMaya.MGlobal.displayError("you need to provide a mesh transform got {wrong} instead".format(wrong = cmds.nodeType(shape)))
        return 
    ...
    rivet = cmds.createNode("MG_polyRivet" , n = baseName)
    ...
    cmds.connectAttr(shape+'.worldMesh',rivet + ".inputSurface" )
    cmds.connectAttr(mesh + '.wm' , rivet + ".meshmeshParentMatrix")
