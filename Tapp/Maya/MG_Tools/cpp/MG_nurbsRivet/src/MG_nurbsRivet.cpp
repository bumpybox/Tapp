...
MObject MG_nurbsRivet::inputNurbsSurface;
...
MStatus MG_nurbsRivet::initialize()
	{ 
		...
		inputNurbsSurface  = typedFn.create("inputNurbsSurface","in",MFnData::kNurbsSurface);
		typedFn.setStorable(true);
		addAttribute(inputNurbsSurface);
		...
		attributeAffects (inputNurbsSurface,output);
		...
		attributeAffects (inputNurbsSurface,outputRotate);
		...
		attributeAffects (inputNurbsSurface,outputMatrix);
...
MStatus MG_nurbsRivet::compute(const MPlug& plug,MDataBlock& dataBlock)
	{
			...
			//Check if the surface is connedted
			MPlug surfPlug( thisMObject(), inputNurbsSurface );
	    
			if (surfPlug.isConnected() == false)
			{	
			...
			//input surface 
			MDataHandle inputNurbsH = dataBlock.inputValue(inputNurbsSurface);
			...
			U = dataBlock.inputValue(uValue).asFloat();
			V = dataBlock.inputValue(vValue).asFloat();
