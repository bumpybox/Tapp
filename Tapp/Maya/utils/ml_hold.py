...
#  /_/ /_/ /_/_/  _________                                   
#               /_________/  Revision 4, 2011-04-30
...
def holdRange(current=False, average=False):
    ...
    with utl.UndoChunk():
        ...
        if mc.keyTangent(query=True, g=True, ott=True)[0]=='linear':
            mc.keyTangent(curves, time=(start,end), itt='linear', ott='linear')
        elif mc.keyTangent(query=True, g=True, ott=True)[0]=='step':
            mc.keyTangent(curves, time=(start,end), itt='plateau', ott='step')
        else:
            mc.keyTangent(curves, time=(start,end), itt='plateau', ott='plateau')
