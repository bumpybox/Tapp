...
MObject MG_polyRivet::meshParentMatrix;
...
MStatus MG_polyRivet::initialize()
	{ 
		...
		meshParentMatrix =matrixFn.create("meshParentMatrix","pmx");
		...
		attributeAffects (meshParentMatrix,output);
		...
		attributeAffects (meshParentMatrix,outputRotate);
		...
		attributeAffects (meshParentMatrix,outputMatrix);
		return MS::kSuccess;
	}
MStatus MG_polyRivet::compute(const MPlug& plug,MDataBlock& dataBlock)
	{
			...
			if (inputG.isConnected()==true)
			{
				...
				MMatrix meshParentMatrixV = dataBlock.inputValue(meshParentMatrix).asMatrix(); 
